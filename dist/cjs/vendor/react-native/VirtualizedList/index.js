...
function extractKey(props, item, index) {
...
function findItemIndexWithKey(props, key) {
  for (var ii = 0; ii < props.getItemCount(props.data); ii++) {
    var _item = props.getItem(props.data, ii);

    var curKey = extractKey(props, _item, ii);
...
function getItemKey(props, index) {
...
class VirtualizedList extends React.PureComponent {
  ...
  constructor(_props) {
      ...
      if (this._isNestedWithSameOrientation()) {
        ...
        return (
          ...
          React.createElement(_ScrollView.default, (0, _extends2.default)({}, props, {
            ...
            contentContainerStyle: _StyleSheet.default.compose(inversionStyle, this.props.contentContainerStyle)
          }))
        );
      } else {
        // $FlowFixMe[prop-missing] Invalid prop usage
        return /*#__PURE__*/React.createElement(_ScrollView.default, (0, _extends2.default)({}, props, {
          contentContainerStyle: _StyleSheet.default.compose(inversionStyle, this.props.contentContainerStyle)
        }));
      }
    };
    ...
    this._onScroll = e => {
      ...
      var maintainVisibleContentPositionAdjustment = this.state.maintainVisibleContentPositionAdjustment;
        ...
        if (!isVirtualizationDisabled) {
          ...
          if (visibleLength > 0 && contentLength > 0) {
            ...
            if (!this.props.initialScrollIndex || this._scrollMetrics.offset) {
              newState = (0, _VirtualizeUtils.computeWindowedRenderLimits)(this.props.data, this.props.getItemCount, maxToRenderPerBatchOrDefault(this.props.maxToRenderPerBatch), windowSizeOrDefault(this.props.windowSize), state, this._getFrameMetricsApprox, this._scrollMetrics); // revert the state if calculations are off
    ...
    this._getFrameMetrics = index => {
      ...
      var frame = item && this._frames[extractKey(this.props, item, index)];
    ...
    this.invertedWheelEventHandler = ev => {
      ...
      var leftoverDelta = delta * SCROLL_WHEEL_SENSITIVITY;
  ...
  static getDerivedStateFromProps(newProps, prevState) {
    var data = newProps.data,
        ...
        maintainVisibleContentPosition = newProps.maintainVisibleContentPosition;
    var prevFirstItemKey = prevState.firstItemKey;
    var maxToRenderPerBatch = maxToRenderPerBatchOrDefault(newProps.maxToRenderPerBatch);
    var maintainVisibleContentPositionAdjustment = prevState.maintainVisibleContentPositionAdjustment;
    var newFirstItemKey = getItemKey(newProps, 0);

    if (maintainVisibleContentPosition != null && maintainVisibleContentPositionAdjustment == null && prevFirstItemKey != null && newFirstItemKey != null) {
      maintainVisibleContentPositionAdjustment = newFirstItemKey !== prevFirstItemKey ? findItemIndexWithKey(newProps, prevFirstItemKey) : null;
  ...
  _pushCells(cells, stickyHeaderIndices, stickyIndicesFromProps, first, last, inversionStyle) {
    ...
    var _loop = function _loop(ii) {
      ...
      var key = extractKey(_this.props, item, ii);
      ...
      _this.pushOrUnshift(cells, /*#__PURE__*/React.createElement(CellRenderer, {
  ...
  render() {
    ...
    if (ListHeaderComponent) {
      ...
      this.pushOrUnshift(cells, /*#__PURE__*/React.createElement(_VirtualizedListContext.VirtualizedListCellContextProvider, {
    ...
    if (itemCount > 0) {
      ...
      var _this$state = this.state,
          ...
          maintainVisibleContentPositionAdjustment = _this$state.maintainVisibleContentPositionAdjustment;
        ...
        if (stickyIndicesFromProps.size > 0) {
          ...
          for (var ii = firstAfterInitial - 1; ii > lastInitialIndex; ii--) {
            if (stickyIndicesFromProps.has(ii + stickyOffset)) {
              ...
              this.pushOrUnshift(cells, /*#__PURE__*/React.createElement(_View.default, {
                key: "$sticky_lead",
                style: {
                  [spacerKey]: leadSpace
                }
  ...
  _maybeCallOnEdgeReached() {
    var _this$props10 = this.props,
        ...
        onStartReached = _this$props10.onStartReached,
        onStartReachedThreshold = _this$props10.onStartReachedThreshold,
        onEndReached = _this$props10.onEndReached,
        onEndReachedThreshold = _this$props10.onEndReachedThreshold,
        initialScrollIndex = _this$props10.initialScrollIndex;
    ...
    var distanceFromEnd = contentLength - visibleLength - offset;
    var startThreshold = onStartReachedThresholdOrDefault(onStartReachedThreshold) * visibleLength;
    var endThreshold = onEndReachedThresholdOrDefault(onEndReachedThreshold) * visibleLength;
    var isWithinStartThreshold = distanceFromStart <= startThreshold;
    var isWithinEndThreshold = distanceFromEnd <= endThreshold;
    var shouldExecuteNewCallback = this._scrollMetrics.contentLength !== this._sentStartForContentLength && this._scrollMetrics.contentLength !== this._sentEndForContentLength; // First check if the user just scrolled within the end threshold
    // and call onEndReached only once for a given content length,
    // and only if onStartReached is not being executed

    if (onEndReached && isWithinEndThreshold && shouldExecuteNewCallback && this.state.last === getItemCount(data) - 1) {
    ...
    else if (onStartReached && isWithinStartThreshold && shouldExecuteNewCallback && this.state.first === 0 && ( // On initial mount when using initialScrollIndex the offset will be 0 initially
  ...
  _scheduleCellsToRenderUpdate() {
    var _this$state2 = this.state,
        ...
        maintainVisibleContentPositionAdjustment = _this$state2.maintainVisibleContentPositionAdjustment;
    ...
    if (first > 0) {
      var distStart = offset - this._getFrameMetricsApprox(first).offset;

      hiPri = hiPri || distStart < 0 || velocity < -2 && distStart < getScrollingThreshold(onStartReachedThreshold, visibleLength);
    ...
    if (last < itemCount - 1) {
      ...
      hiPri = hiPri || distBottom < 0 || velocity > 2 && distBottom < getScrollingThreshold(onEndReachedThreshold, visibleLength);
